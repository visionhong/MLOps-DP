...
class PytorchImagePreprocessTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: Union[Image.Image, np.ndarray]) -> np.ndarray:
        if isinstance(X, np.ndarray):
            dim = self.image_size + (3,)
            if X.shape != dim:
                raise ValueError(f"resize to image_size {self.image_size} beforehand for numpy array")
        else:
            X = np.array(X.resize(self.image_size))

        image_data = X.transpose(2, 0, 1).astype(np.float32)  # (224, 224, 3) -> (3, 224, 224)
        mean_vec = np.array(self.mean_vec)  # 평균
        stddev_vec = np.array(self.stddev_vec)  # 표준편차
        norm_image_data = np.zeros(image_data.shape).astype(np.float32)  # 초기화
        for i in range(image_data.shape[0]):  # channel 별로 normalize
            norm_image_data[i, :, :] = (image_data[i, :, :] / 255 - mean_vec[i]) / stddev_vec[i]
        norm_image_data = norm_image_data.reshape(self.prediction_shape).astype(np.float32)  # batch 추가
